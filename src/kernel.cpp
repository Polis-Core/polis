...
{
    std::shared_ptr<const CTransaction> tx = block.vtx[1];
    if (!tx->IsCoinStake())
    ...
    uint256 hashBlock;
    std::shared_ptr<const CTransaction> txPrev;
...
//            std::cout << txPrev.ToString() << std::endl;
    ...
    if (!VerifyScript(txin.scriptSig, txPrev->vout[txin.prevout.n].scriptPubKey, STANDARD_SCRIPT_VERIFY_FLAGS, TransactionSignatureChecker(&uTx, 0)))
